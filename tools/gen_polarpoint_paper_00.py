...
SaveJson  = False       # 是否保存极点标签为.json文件
SaveOverlay = True      # 只保存“透明极点叠加层”（用于PPT叠加）
OverlayDPI  = 300       # PPT里缩放更清晰（200~600都行）


# 本地
data_root = "/home/liulei/ll/PPBEV_ll/data"
root_dirs  = ["/home/liulei/ll/PPBEV_ll/data/town01_val_00"]   # ⚠️ 修改: 需要处理的文件夹(每次修改这里)
# 服务器
...
def process_one(front_img_path, vehicles_info, ego_pos=(96, 152), ego_yaw=0.0, scale=2.0, r_max=100, future_vehicles_info=None, time_weights=None):
    ...
    if SaveOverlay:
        # -----------------------------
        # A) 透明叠加层：对齐 bev_left (192x192) 的极点 overlay
        # -----------------------------
        H_left, W_left, _ = bev_left.shape

        fig = plt.figure(figsize=(W_left / 100.0, H_left / 100.0), dpi=100)
        ax = fig.add_axes([0.0, 0.0, 1.0, 1.0])
        ...
        # 像素坐标系对齐：左上为(0,0)
        ax.set_xlim(0, W_left)
        ax.set_ylim(H_left, 0)
        ax.axis("off")
        ...
        ax.scatter(
            ...
            c=risks,
            cmap=cmap,
            s=8,  # PPT叠加一般要比你原来的s=5更明显
            vmin=0, vmax=1,
            linewidths=0
        )

        overlay_dir = os.path.join(os.path.dirname(os.path.dirname(bev_path)), "pole_overlay_png_25x36")
        os.makedirs(overlay_dir, exist_ok=True)
        overlay_path = os.path.join(overlay_dir, os.path.basename(bev_path))  # 同名png

        fig.savefig(
            overlay_path,
            transparent=True,  # 再次确保透明
            dpi=OverlayDPI,  # 更高清，PPT缩放不糊
            bbox_inches="tight",
            pad_inches=0
        )
        plt.close(fig)
        ...
        # -----------------------------
        # B) 如果你也要对齐 bev_rgb(1024x1024) 底图：导出 overlay（可选）
        #    若PPT底图是 bev_rgb，请开启这一段
        # -----------------------------
        ...
        if os.path.exists(bev_rgb_path):
            ...
            offset_bev = np.zeros_like(poles, dtype=np.float32)
            offset_bev[:, 0] = poles[:, 0] - ego_x_bev
            offset_bev[:, 1] = poles[:, 1] - ego_y_bev
            offset_m = offset_bev / PIXELS_PER_METER
            offset_rgb = offset_m / meters_per_pix_rgb
            ...
            ax.scatter(
                ...
                c=risks,
                cmap=cmap,
                s=18,  # 1024图更大，点也可以适当大一些
                vmin=0, vmax=1,
                linewidths=0
            )

            overlay_rgb_dir = os.path.join(os.path.dirname(os.path.dirname(bev_rgb_path)), "pole_overlay_rgb_png_25x36")
            os.makedirs(overlay_rgb_dir, exist_ok=True)
            overlay_rgb_path = os.path.join(overlay_rgb_dir, os.path.basename(bev_rgb_path))
