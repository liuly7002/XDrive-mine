...
class TrafficLightHandler:
    ...
    def get_stopline_vtx(veh_loc, color, dist_threshold=50.0):
        """
        根据自车位置，获取附近处于指定灯色状态的交通灯对应的停车线坐标集合（stopline vertex）
        """
        if color == 0:     # 绿灯
            tl_state = carla.TrafficLightState.Green
        elif color == 1:   # 黄灯
        ...
        stopline_vtx = []
        for i in range(TrafficLightHandler.num_tl):                # 遍历所有交通灯(通过 TrafficLightHandler.list_tl_actor)
            traffic_light = TrafficLightHandler.list_tl_actor[i]
            tv_loc = TrafficLightHandler.list_tv_loc[i]
            if tv_loc.distance(veh_loc) > dist_threshold:          # 条件1: 获取交通灯的位置（list_tv_loc[i]）并与自车位置做距离过滤
                continue
            if traffic_light.state != tl_state:                    # 条件2: 判断该交通灯是否是指定颜色的状态
                continue
            stopline_vtx += TrafficLightHandler.list_stopline_vtx[i]  # 如果满足以上两个条件，提取该红绿灯的停车线坐标 list_stopline_vtx[i]

        return stopline_vtx
        # stopline_vtx = [carla.Location(x1, y1, z1), carla.Location(x2, y2, z2), ...]
        # stopline_vtx 是一个 坐标点的列表，这些点表示在 Carla 世界坐标系下的停车线（Stopline）顶点坐标
