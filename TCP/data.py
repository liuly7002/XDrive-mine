...
class CARLA_Data(Dataset):
	...
	def __init__(self, root, data_folders, img_aug = False):
		...
		self.route_world = []       # 参考路径20个点在新坐标系下的位置(所有样本)
		self.max_len = 20           # 最大路径点数
		...
		for sub_root in data_folders:
			...
			self.route_world += data['route_world']         # 全局参考路径的点(<=20个)在新坐标系下坐标
			self.polar_point += data['polarpoint_labels']   # 极点

			self.x_command += data['x_target']      # 下一个需要跟踪的路径点x值(新坐标系下)
			self.y_command += data['y_target']      # 下一个需要跟踪的路径点y值(新坐标系下)
			self.command += data['target_command']  # 下一个需要跟踪的路径点所包含的高级导航指令

			self.front_img += data['front_img']     # 前视相机图像
			self.x += data['input_x']               # 自车当前在新坐标系下的x坐标
			self.y += data['input_y']               # 自车当前在新坐标系下的y坐标
			self.theta += data['input_theta']       # 自车当前在新坐标系下的航向角
			self.speed += data['speed']             # 自车当前的车速

			self.future_x += data['future_x']         # 自车未来连续4帧的x坐标(新坐标系下)
			self.future_y += data['future_y']         # 自车未来连续4帧的y坐标(新坐标系下)
			self.future_theta += data['future_theta'] # 自车未来连续4帧的航向角

			self.future_feature += data['future_feature']
			self.future_action += data['future_action']      # 未来4帧的[throttle, steer, brake]
			...
			self.action += data['action']             # [throttle, steer, brake]
			self.action_mu += data['action_mu']
			self.action_sigma += data['action_sigma']
			self.only_ap_brake += data['only_ap_brake']   # False 或者 True  True代表:系统判断应该刹车，但车辆没有真正刹车  所以False是好的
	...
	def __getitem__(self, index):
		...
		ego_x = self.x[index][0]         # 自车当前在新坐标系下的x值
		ego_y = self.y[index][0]         # 自车当前在新坐标系下的y值
		ego_theta = self.theta[index][0] # 自车当前在新坐标系下的航向角

		# 将自车未来的4帧位置从新坐标系下转换到自车坐标系
		waypoints = []
		for i in range(4):
			R = np.array([
		...
		data['waypoints'] = np.array(waypoints)  # 这是自车未来4帧的位置,在自车坐标系下的位置
		...
		for i in range(len(self.route_world[index])):
			for point in self.route_world[index][i]:
				px, py = point
		...
		if actual_len == 0:
			# 避免空数组报错，用零填充
			routepoints = np.zeros((self.max_len, 2))
			actual_len = 0
		...
		data['routepoints'] = routepoints                # Padding到20个点的路径点
		data['route_len'] = actual_len                   # 路径原始长度

		# ========== 极点 ==========
		polar_json_path = os.path.join("./data", self.polar_point[index][0].lstrip("/"))
		with open(polar_json_path, "r") as f:
			polar_data = json.load(f)
		polar_tensor = torch.tensor(polar_data, dtype=torch.float32)    # shape = [25, 36]
		polar_tensor = polar_tensor.unsqueeze(0)                              # shape = [1,25,36]
		data['polarpoints'] = polar_tensor
		...
		# 将下一个需要跟踪的路径点x值y值从新坐标系下转换到自车坐标系
		R = np.array([
			[np.cos(np.pi/2+ego_theta), -np.sin(np.pi/2+ego_theta)],
			[np.sin(np.pi/2+ego_theta),  np.cos(np.pi/2+ego_theta)]
			])
		...
		data['target_point'] = local_command_point_aim[:2]  # 转换到这里
