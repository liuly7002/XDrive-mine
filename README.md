...
```
git clone https://github.com/liuly7002/XDrive-mine.git
cd XDrive-mine
conda env create -f environment.yml --name XDriveMine
conda activate XDriveMine
```
...
```
2.运行 tools 目录下的 gen_riskmap.py 文件, 命令行如下:
```
python tools/gen_riskmap.py
```
备注,需要修改的内容如下:
```
...
3.运行 tools 目录下的 gen_data.py 文件, 命令行如下:
```
